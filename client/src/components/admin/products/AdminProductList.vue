    ...
    <div class="content">
      <div class="search row py-3 pl-3">
        ...
        <div class="sort offset-4 col-sm-4">
          <div class="row">
            <select name="" id="" class="form-control col-3" @change="showCount">
            ...
            <div class="page-info col-md-4 pt-2">
              <span
                >Hiển thị {{ (this.currentPage - 1) * this.count_view + 1 }}-{{
                  this.currentPage * this.count_view > this.total
                    ? this.total
                    : this.currentPage * this.count_view
                }}/{{ this.total }}</span
              >
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    nextPage() {
      if (this.total > this.currentPage * this.count_view) {
        this.currentPage++;
        this.products = store.state.product.products
          .filter((item) => {
            return item.name.includes(this.text_search);
          }).slice(
          (this.currentPage - 1) * this.count_view,
          this.currentPage  * this.count_view
        );
      }
    },
